...
(
	...
	WHERE "USER".name = 'Q太郎'),
	(select id from "SKILL" WHERE "SKILL".name = '有氧運動')
),
(
	(select "COACH".id from "COACH"
	inner join "USER" on "USER".id = "COACH".user_id
	WHERE "USER".name = 'Q太郎'),
	(select id from "SKILL" WHERE "SKILL".name = '復健訓練')
),
(
	(select "COACH".id from "COACH"
	inner join "USER" on "USER".id = "COACH".user_id
	WHERE "USER".name = '肌肉棒子'),
	(select id from "SKILL" WHERE "SKILL".name = '瑜伽')
);
...
select
    ...
    '即將授課'
from "USER"
where "USER".email ='richman@hexschooltest.io';
...
from "USER"
where "USER".email ='wXlTq@hexschooltest.io'
and "USER".id = "COURSE_BOOKING".user_id;
...
FROM
    "COURSE_BOOKING" cb
WHERE
    cb.created_at BETWEEN '2024-11-01 00:00:00' AND '2024-11-30 23:59:59'
    AND cb.status != '課程已取消';